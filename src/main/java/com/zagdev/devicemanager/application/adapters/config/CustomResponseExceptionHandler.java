...
public class CustomResponseExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleTypeMismatch(TypeMismatchException ex, HttpHeaders headers, HttpStatusCode status, WebRequest request) {
        ...
        logger.info("Invalid request to [{}]. Parameter [{}] with value [{}] is not of expected type [{}].", endpoint, parameterName, invalidValue, expectedType);

        String errorMessage = String.format("Parameter '%s' with value '%s' is not of expected type '%s'.", parameterName, invalidValue, expectedType);

        List<ResponseError.FieldError> fieldErrors = new ArrayList<>();
        fieldErrors.add(new ResponseError.FieldError(parameterName, new ArrayList<>(List.of(errorMessage))));

        return new ResponseEntity<>(ResponseError.build(ErrorCode.INVALID_DATA, fieldErrors), HttpStatus.BAD_REQUEST);
    }
